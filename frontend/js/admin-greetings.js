...
function drawCrop() { if (!cropImage) return; const ctx = cropCanvas.getContext('2d'); const base = Math.max(cropCanvas.width / cropImage.width, cropCanvas.height / cropImage.height); const drawWidth = cropImage.width * base; const drawHeight = cropImage.height * base; const dx = (cropCanvas.width - drawWidth) / 2; const dy = (cropCanvas.height - drawHeight) / 2; ctx.clearRect(0, 0, cropCanvas.width, cropCanvas.height); ctx.fillStyle = '#050505'; ctx.fillRect(0, 0, cropCanvas.width, cropCanvas.height); ctx.drawImage(cropImage, dx, dy, drawWidth, drawHeight); syncCropBox(); }
...
async function showCrop() { if (cropIndex >= cropQueue.length) { cropModal.hidden = true; field('photo').value = ''; renderPhotoPreview(); if (await saveCroppedPhotosDirectly()) return; setStatus(`${croppedFiles.length} foto siap disimpan ke panel.`); return; } cropCounter.textContent = `Foto ${cropIndex + 1} dari ${cropQueue.length}: atur kotak crop`; cropImage = await createImageBitmap(cropQueue[cropIndex]); cropSelection = { x:cropCanvas.width * .12, y:cropCanvas.height * .12, w:cropCanvas.width * .76, h:cropCanvas.height * .76 }; drawCrop(); }
