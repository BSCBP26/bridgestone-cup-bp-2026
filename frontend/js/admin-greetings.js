...
async function showCrop() { if (cropIndex >= cropQueue.length) { cropModal.hidden = true; field('photo').value = ''; setStatus(`${croppedFiles.length} foto siap disimpan ke panel. Klik Simpan Exhibition Match.`); return; } cropCounter.textContent = `Foto ${cropIndex + 1} dari ${cropQueue.length}: atur kotak crop`; cropImage = await createImageBitmap(cropQueue[cropIndex]); cropSelection = { x:cropCanvas.width * .12, y:cropCanvas.height * .12, w:cropCanvas.width * .76, h:cropCanvas.height * .76 }; drawCrop(); }
