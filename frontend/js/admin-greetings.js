...
const photoLabel = field('photo').previousElementSibling?.querySelector('small');
if (photoLabel) photoLabel.textContent = 'Maksimal 20 foto · bisa pilih satu atau beberapa';
...
async function saveCroppedPhotosDirectly() {
  ...
  try {
    ...
    await adminRequest(`/admin/exhibition-matches/${current.id}`, { method:'PUT', body:JSON.stringify({ ...current, photoStoragePaths:existing.concat(uploaded).slice(0, 20) }) });
...
async function openCropQueue(files) { const current = greetings.find(item => item.id === field('id').value); const existing = current?.photoStoragePaths || (current?.photoStoragePath ? [current.photoStoragePath] : []); const limit = appendPhotos ? Math.max(0, 20 - existing.length) : 20; cropQueue = files.slice(0, limit); croppedFiles = []; cropIndex = 0; cropModal.hidden = false; await showCrop(); }
...
form.addEventListener('submit', async event => {
  ...
  try {
    ...
    if (files.length) {
      ...
      photoStoragePaths = (appendPhotos ? photoStoragePaths : []).concat(uploaded.storagePaths).slice(0, 20);
    }
