...
let cropQueue = [], croppedFiles = [], cropIndex = 0, cropImage = null;
function drawCrop() { if (!cropImage) return; const ctx = cropCanvas.getContext('2d'); const zoom = Number(cropZoom.value); const targetRatio = cropCanvas.width / cropCanvas.height; const sourceRatio = cropImage.width / cropImage.height; let sw = sourceRatio > targetRatio ? cropImage.height * targetRatio : cropImage.width; let sh = sourceRatio > targetRatio ? cropImage.height : cropImage.width / targetRatio; sw /= zoom; sh /= zoom; const sx = (cropImage.width - sw) / 2; const sy = (cropImage.height - sh) / 2; ctx.clearRect(0, 0, cropCanvas.width, cropCanvas.height); ctx.drawImage(cropImage, sx, sy, sw, sh, 0, 0, cropCanvas.width, cropCanvas.height); }
async function openCropQueue(files) { cropQueue = files; croppedFiles = []; cropIndex = 0; cropModal.hidden = false; await showCrop(); }
async function showCrop() { if (cropIndex >= cropQueue.length) { cropModal.hidden = true; field('photo').value = ''; return; } cropCounter.textContent = `Foto ${cropIndex + 1} dari ${cropQueue.length}: ${cropQueue[cropIndex].name}`; cropZoom.value = '1'; cropImage = await createImageBitmap(cropQueue[cropIndex]); drawCrop(); }
