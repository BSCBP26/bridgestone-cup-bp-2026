...
const dateLabel = date => new Intl.DateTimeFormat('id-ID', { day:'2-digit', month:'short', timeZone:'Asia/Jakarta' }).format(new Date(`${date}T00:00:00+07:00`));
...
function activityRows(rows) { return rows.length ? rows.map(row => `<tr><td>${runnerLink(row.name)}</td><td>${dateLabel(row.date)} 2026</td><td>${formatKm(row.distanceKm)}</td><td>${formatDuration(row.durationSeconds)}</td><td>${formatPace(row.paceSeconds)}</td><td><span class="recorded-status">Tercatat</span></td></tr>`).join('') : '<tr><td colspan="6">Aktivitas tidak ditemukan.</td></tr>'; }

function render(data) {
  const { summary, leaders, runners, activities=[] } = data;
