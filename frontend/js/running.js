...
function dailyChartMarkup(daily) {
  const visible = [...daily].sort((a,b) => a.date.localeCompare(b.date)).slice(-14);
  ...
  const nodes=points.map(point=>`<g class="daily-point"><title>${escapeHtml(`${dateLabel(point.date)}: ${formatKm(point.totalKm)}, ${point.activities} aktivitas`)}</title><circle cx="${point.x}" cy="${point.y}" r="6"/><text x="${point.x}" y="${height-18}" text-anchor="middle">${escapeHtml(new Intl.DateTimeFormat('id-ID',{day:'numeric',month:'short',timeZone:'Asia/Jakarta'}).format(new Date(`${point.date}T00:00:00+07:00`)))}</text></g>`).join('');
  return `<section class="running-chart-panel"><header class="running-chart-head"><div><small>AKTIVITAS RUNNING CLUB</small><h2>JARAK HARIAN</h2><p>Total kilometer dari aktivitas yang tercatat pada tiap tanggal.</p></div><span class="chart-total">${visible.length} TANGGAL TERAKHIR</span></header><div class="running-chart-scroll"><svg class="running-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="Grafik total kilometer aktivitas per tanggal"><defs><linearGradient id="dailyArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#e4b33f" stop-opacity=".32"/><stop offset="1" stop-color="#e4b33f" stop-opacity="0"/></linearGradient></defs>${grid}<path class="daily-area" d="${area}"/><path class="line-path" d="${line}"/>${nodes}</svg></div><footer><span class="chart-legend-dot"></span>Total KM per tanggal &nbsp;·&nbsp; Arahkan kursor ke titik untuk detail.</footer></section>`;
}

function render(data) {
  const { summary, leaders, runners, daily=[], activities=[] } = data;
  const recentActivities=[...activities].sort((a,b)=>b.date.localeCompare(a.date)).slice(0,12);
  ...
  <div id="running-recent-view" hidden>${dailyChartMarkup(daily)}<section class="running-table-panel"><header class="running-table-head"><div><small>CATATAN TERBARU</small><h2>AKTIVITAS TERBARU</h2><p>${recentActivities.length} aktivitas terbaru berdasarkan tanggal.</p></div></header><div class="running-table-scroll"><table class="running-table activity-table"><thead><tr><th>PELARI</th><th>TANGGAL</th><th>JARAK</th><th>DURASI</th><th>PACE</th><th>STATUS</th></tr></thead><tbody>${activityRows(recentActivities)}</tbody></table></div></section></div>`;
