...
export async function deleteGreeting(id,client=getSupabaseAdminClient()) { const {data,error}=await client.from("greetings").delete().eq("id",id).select("id, photo_storage_path, photo_storage_paths").single(); if(error||!data) throw new AppError(404,"Greeting not found"); return {id:data.id,photoStoragePath:data.photo_storage_path,photoStoragePaths:data.photo_storage_paths || (data.photo_storage_path ? [data.photo_storage_path] : [])}; }
